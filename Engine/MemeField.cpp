...
MemeField::MemeField(int nMemes)
{
	...
	for (Vei2 gridPos = { 0,0 }; gridPos.y < height; gridPos.y++)
	{
		...
		{
			TileAt(gridPos).SetNeighborMemesCount(CountNeighborMemes(gridPos));
		}
	}
}
...
void MemeField::Draw(Graphics & gfx) const
{
	...
	for (Vei2 gridPos = { 0,0 }; gridPos.y < height; gridPos.y++)
	{
		for (gridPos.x = 0; gridPos.x < width; gridPos.x++)
		{
			TileAt(gridPos).Draw(gridPos * SpriteCodex::tileSize,isBombed, gfx);
		}
	}
}
...
void MemeField::Tile::Draw(const Vei2& screenPos, bool bombed, Graphics & gfx) const
{
	if (!bombed)
	{
		switch (state)
...
void MemeField::Tile::SetNeighborMemesCount(int nMemeCount)
{
